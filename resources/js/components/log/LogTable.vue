  ...
  <div class="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
    <div class="flex items-center gap-4">  
        ...
        <div class="px-4 pt-3 pb-3">
            <Button 
                @click="show = !show" 
                class="bg-blue-600 text-white rounded hover:bg-blue-700 transition"                
            >
                {{ show ? 'Hide' : 'Show' }}
            </Button>
        </div>
    ...
    <table v-if="show" class="min-w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          ...
          <th class="px-6 py-3 text-left tracking-wider">Description</th>
      ...
      <tbody class="divide-y divide-gray-200 bg-white">
        ...
        <tr v-for="item in items" :key="item.id" class="transition-colors hover:bg-gray-50">
          ...
          <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-700 capitalize">
            {{ item.description }}
          </td>
          <td class="px-6 py-4 text-sm">
            <div v-if="item.changes?.attributes" class="flex flex-col gap-1">
              <LogOldNew :log="item.changes" />
            </div>
            <div v-else-if="item.properties?.quizzes" class="flex flex-col gap-1">
              <LogTableQuizzes 
                :quizzes="item.properties.quizzes" 
              />
            </div>
            <div v-else-if="item.properties?.answers" class="flex flex-col gap-1">              
              <LogTableAnswers 
                :answers="item.properties.answers"
              />              
            </div>
            <div v-else-if="item.properties?.tags" class="flex flex-col gap-1">              
              <LogTableTags
                :tags="item.properties.tags"
              />              
            </div>            
            <span v-else class="text-gray-400 italic">No tracked changes</span>
          </td>
